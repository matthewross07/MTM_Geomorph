...
## Figure 1 - Mining demonstration

The images from this figure come from an aerial flight done by Duke University.
The DEM generation is below. 

```{r, eval = F}
old.e <- raster('data/in/olddem_Clip1.tif')

new.e <- raster('data/in/lidshedDEMcl1.tif')

summary(new.e)
#Read in Elev Rasters
new.e <- raster('lidshedDEMcl1.tif')
old.e <- raster('olddem_Clip1.tif')

#Set min surface value
new.e[is.na(new.e)] <- minValue(new.e)
old.e[is.na(old.e)] <- minValue(old.e)

#Setup old elevation profile matrix
old.m <- as.matrix(old.e)
old.x <- 10*1:nrow(old.m)
old.y <- 10*1:ncol(old.m)
old.z <- 2*as.matrix(old.m)

#Setup new elevation profile as matrix
new.m <- as.matrix(new.e)
new.x <- 10*1:nrow(new.m)
new.y <- 10*1:ncol(new.m)
new.z <- 2*as.matrix(new.m)

persp(old.x,old.y,old.z,scale=F,box=F,theta=-10,phi=30)
...
#Frist breaks are befrore 10^4 by visual inspection and lit values.
#First minimum is generally the biggest minimum but not for all sites particularly post-mining
...
channel_heads <- read_csv('data/in/channel_heads.csv')
...
super_plotter <- function(spruce = all.list[[10]],
  ...
  png(filename=filename,width=12,height=10,units='in',res=300)
    ...
    par(mar=c(0,0,2,0),mgp=c(1.8,0.5,0),cex=1.5,cex.lab=1.5,cex.axis=1.3)
    ...
    area = round(length(new.m.na)/(10000),0)
    
    label = bquote(.(sites[i]) ~ '-' ~ .({area}) ~ km^2)
    ...
    par(mar=hist.mar)
    hist(old.m.na,breaks=seq(zlims[1],zlims[2],length.out=50),xlim=zlims,col='blue',ylim=c(0,7000),xlab='Elevation (m)',ylab='Frequency',main='')
    ...
    if(sites[i] %in% c('WhiteOak', 'Laurel','Ben')){
    hist(old.s,breaks=seq(0,4,length.out=150),xlim=c(0,1.2),col='blue',ylim=c(0,10000),xlab='Slope (m/m)',ylab='Frequency',main='')
    hist(new.s, breaks=seq(0,4,length.out=150),xlim=c(0,1.2),col=reds2,add=T,density=40)
        text(x = 0.1, y = 9000,
         labels = c('B'),
         cex = 4)
    }else{
         hist(old.s,breaks=seq(0,4,length.out=100),xlim=c(0,1.2),col='blue',ylim=c(0,8000),xlab='Slope (m/m)',ylab='Frequency',main='')
    ...
    par(mar=hist.mar)
    hist(old.f.m,breaks=seq(1,6.05,length.out=100),col='blue',ylim=c(0,8000),
    ...
    legend(x=3,y=4000,pch=c(15,15),col=c('blue',reds2),legend=c('Pre-Mining','Post-Mining'),cex=1.5)
    text(x = 2, y = 7000,
         labels = c('C'),
         cex = 3)
    ...
    par(mar=hist.mar)
    hist(old.ei,breaks=seq(-2.1,6,length.out=100),col='blue',ylim=c(0,8000),
...
for(i in 1:length(sites)){
  print(sites[i])
  filename = paste0('Figures2018/3d_indices/',sites[i],'_3D_4_indices.png')
...
drape <- function(site = 'Spruce',
  ...
  if(site == 'WhiteRef' & type == 'Unmined'){
    site1 = 'WhiteOakRef'
  }else{site1 = site}
  
  unique(sa.breaks$Site)
  sa.new <- sa.breaks %>% 
    dplyr::filter(Site == site1,
           Type == type,
           Data.Source == 'Current')
  ...
  png(filename=filename,width=9,height=4,units='in',res=300)
    ...
    persp3D(x=x, y=y,z=old.m.na,col=zone.cols,
            ...
            scale = FALSE, shade = 0.6,clab='Domain',expand=2,
    ...
    if(site != 'Spruce'){
    mtext(label, side = 3, line = -3,
          cex = 1)
    }
    
    ## Count histogram
    
    hmax = ceiling(max(c(table(newzones),table(oldzones)))/10000)*10000
    hseq = seq(0,hmax,length.out = 3)
    ...
    hist(oldzones,col='blue',
      ...
      main='',xaxt='n',yaxt='n',
      ylim = c(0,hmax))
    axis(1,at=c(1,2,3,4),padj=0,line=-.8)
    axis(2, at = hseq,labels =  round(hseq/10000,1))
    hist(newzones,col=reds2,
      xlab='Domain',ylab='Frequency',main='',xaxt='n',density=40,add=T)
...
for(i in 1:length(sites)){
  print(sites[i])
  filename = paste0('Figures2018/3d_drape/',sites[i],'Zone_Drape.png')
...
sa.zones_stats
write_csv(sa.zones_stats,'Figures2018/zonal_stats.csv')


wide_slope <- sa.zones_stats %>%
  select(Watershed, Type, Data.Source, zones, median_slope) %>%
  mutate(median_slope = round(median_slope,2)) %>%
  pivot_wider(names_from = Data.Source,
              values_from = median_slope) %>%
  mutate(percent_change = round(100*((Current - Historic)/Historic),0)) %>%
  mutate(slope = paste0(Historic,'-',Current,' (',percent_change,'%)')) %>%
  select(Watershed, zones, Type, slope)  %>%
  mutate(zones = as.character(zones)) %>%
  pivot_longer(cols = slope) %>%
  pivot_wider(names_from = zones) %>%
  select(-name)
  

write_csv(wide_slope, 'Figures2018/zone_slope_stats.csv')
```
