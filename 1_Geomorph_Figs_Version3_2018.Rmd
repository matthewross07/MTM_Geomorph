...
# Raw site summaries (median elevation, total relief, etc...)  for changes on old elevation profile

## %mining
```{r}
#Read in mining detection layer
...
pmine.list <- foreach(i=1:length(all.list)) %dopar% {
  ...
  mine.area = length(mine.mask[mine.mask < 2015])
...
e.wide <- elev.summary %>%
  ...
  arrange(Site) %>% 
  mutate(elev.change= Historic-Current)%>%
...
area.wide <- elev.summary %>%
  ...
  arrange(Site) %>%
  mutate(area.change= Historic-Current)%>%
...
s.wide <- slope.summary %>%
  ...
  dplyr::select(Site,Type,Data.Source,median) %>%
  spread(Data.Source,median) %>%
  mutate(slope.change= Historic-Current)%>%
  arrange(Site) %>%
...
I'm pretty sure previous work has identified 100 pixels has channel heads so that would be 10,000 m2 
```{r}
...
d.wide <- drainage.density %>%
  ...
  mutate(DD.change= Historic-Current)%>%
  arrange(Site) %>%
...
```{r}
sa.long <- uaa.dif %>%
  select(Site,Type,Data.Source,data=pred,first=derv.mod,second=derv2,UAA) %>%
  mutate(data=log10(data)) %>%
  gather(key=derivative,value=value,-Site,-Type,-Data.Source,-UAA)


pdf(file='Figures2018/SA.All.Derv.Mined.4.4.pdf',width=10,height=8)
sa.long %>%
  filter(Type == 'Mined') %>%
  ggplot(.,aes(x=UAA,y=value,color=Data.Source)) + 
  geom_line(size=1) + 
    geom_vline(data=sa.mine.breaks,aes(xintercept=value,color=Data.Source)) + 
  facet_grid(derivative~Site,scales='free_y') + 
...
pdf(file='Figures2018/SA.All.Derv.UnMined.pdf',width=10,height=8)
sa.long %>%
  filter(Type != 'Mined') %>%
  ggplot(.,aes(x=UAA,y=value,color=Data.Source)) + 
  geom_line(size=1) + 
  ...
  geom_vline(data=sa.unmine.breaks,aes(xintercept=value,color=Data.Source)) + 
  theme(legend.position='top',legend.direction='horizontal') + 
    ylab('Slope (m/m)') +
  xlab(expression(paste('Area (',m^2,')'))) + 
    scale_x_log10(breaks=trans_breaks("log10", function(x) 10^x),
...
## Individual UAA plots
```{r, eval=F}
#plot with mined sites only 
pdf(file='Figures2018/UAA.Slope.Mined.pdf',width=10,height=4)
aa.g <- uaa %>% 
  filter(Type == 'Mined') %>%
  ggplot(aes(x=UAA,y=Slope,color=Data.Source)) + 
  stat_smooth(se=F,method='loess',size=.6,span=.5) + 
  geom_point(shape=1) + 
...
pdf(file='Figures2018/UAA.Slope.UnMined.pdf',width=10,height=4)
aa.ug <- uaa %>% 
  filter(Type != 'Mined') %>%
  ggplot(aes(x=UAA,y=Slope,color=Data.Source)) + 
  stat_smooth(se=F,method='loess',size=.6,span=.5) + 
...
pdf(file='Figures2018/UAA.Slope.Derv.Mined.pdf',width=10,height=4)
...
pdf(file='Figures2018/UAA.Slope.Derv.UnMined.pdf',width=10,height=4)
...
cad.breaks <- bind_rows(diffusecad,incisioncad)
cad %>% 
ggplot(.,aes(x=UAA,y=Cume_Freq,color=Data.Source)) + 
  geom_point(shape=1) + 
  scale_x_log10() + 
  ...
  facet_wrap(~Site,ncol=5) + 
  geom_vline(data=cad.breaks,aes(xintercept=value,color=Data.Source)) + 
  theme(legend.position = 'top',legend.direction = 'horizontal')
