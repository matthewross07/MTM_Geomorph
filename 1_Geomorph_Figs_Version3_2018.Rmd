...
sa.mine.old.breaks <- sa.mine.breaks %>%
...
mined.sa <- uaa %>%
  ...
  geom_vline(data=sa.mine.old.breaks,aes(xintercept=value,color=Data.Source)) + 
...
sa.unmine.new.breaks <- sa.unmine.breaks %>%
  filter(Data.Source != 'Current')
...
unmined.sa <- uaa %>%
  ...
  geom_point(size=1) + 
  stat_smooth(method='loess',span=0.4,se=F) + 
  geom_vline(data=sa.unmine.new.breaks,aes(xintercept=value,color=Data.Source)) + 
  facet_grid(Type~Site,scales='free_y') + 
  scale_color_manual(name='Data Source',values=c('Orange3','Purple4')) + 
  theme(legend.position='top',legend.direction='horizontal') + 
  ylab('Slope (m/m)') +
  xlab(expression(paste('Area (',m^2,')'))) + 
  scale_x_log10(breaks=trans_breaks("log10", function(x) 10^x),
...
ggarrange(mined.sa,unmined.sa,ncol=1,nrow=2,heights=c(1,1.1),legend='top')
