...
full.uaa <- tibble(UAA=10^seq(2,7,.01)) %>% nest() %>% as.list()
...
spruce.sa.new <- sa.breaks %>% 
  filter(Site == 'Spruce',
         Type=='Mined',
         Data.Source == 'Current')


spruce.sa.old <- sa.breaks %>% 
    filter(Site == 'Spruce',
         Type=='Mined',
         Data.Source == 'Historic')


newzones <- cut(newfac,breaks=c(0,spruce.sa.new$value/10,maxValue(newfac))) %>%
  as.matrix(.) %>% as.integer()



oldzones <- cut(oldfac,breaks=c(0,spruce.sa.old$value/10,maxValue(oldfac))) %>%
  as.matrix(.) %>% as.integer()
...
png(filename='Figures2018/3D_drape.png',width=9,height=4,units='in',res=300)
par(mar=c(0,0,0,0),mgp=c(1.8,0.5,0),cex=1.5,cex.lab=1.5,cex.axis=1.3)
